...
**UMCP v2.2.5** · **14,896 tests** · **20 domains** · **181 closure modules** · **47 lemmas** · **44 structural identities**
...
pytest                                       # 14,896 tests (pytest --collect-only | grep ":" | wc -l to verify)
...
**14,896 test cases** across **174 test files** in `tests/` (173 top-level `test_*.py` + 1 in `tests/closures/` + `conftest.py`), numbered by tier and domain (`test_000_*` through `test_293_*`). Single `tests/conftest.py` provides:
...
- Parametrized tests expand the collected items to 14,896 (verify: `pytest --collect-only | grep "::" | wc -l`)
