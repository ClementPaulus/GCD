import{c as gt,g as Rt,a as Tt}from"./kernel.C-ROa8s-.js";import{B as I,r as wt,p as Ft,s as zt,i as vt,o as It,b as Lt,a as _t,f as Bt,h as Dt,e as kt,g as Vt}from"./spacetime.CHHUQeVA.js";import"./constants.6x37F0HI.js";function tt(){return new Float32Array(16)}function Ut(){const e=tt();return e[0]=e[5]=e[10]=e[15]=1,e}function Ot(e,n,l,t){const r=tt(),s=1/Math.tan(e*.5),a=1/(l-t);return r[0]=s/n,r[5]=s,r[10]=(t+l)*a,r[11]=-1,r[14]=2*t*l*a,r}function Ht(e,n){const l=tt();for(let t=0;t<4;t++)for(let r=0;r<4;r++)l[r*4+t]=e[t]*n[r*4]+e[4+t]*n[r*4+1]+e[8+t]*n[r*4+2]+e[12+t]*n[r*4+3];return l}function N(e){const n=Math.sqrt(e[0]*e[0]+e[1]*e[1]+e[2]*e[2])||1;return[e[0]/n,e[1]/n,e[2]/n]}function Y(e,n){return[e[1]*n[2]-e[2]*n[1],e[2]*n[0]-e[0]*n[2],e[0]*n[1]-e[1]*n[0]]}function Gt(e,n,l){const t=N([e[0]-n[0],e[1]-n[1],e[2]-n[2]]),r=N(Y(l,t)),s=Y(t,r),a=Ut();return a[0]=r[0],a[4]=r[1],a[8]=r[2],a[12]=-(r[0]*e[0]+r[1]*e[1]+r[2]*e[2]),a[1]=s[0],a[5]=s[1],a[9]=s[2],a[13]=-(s[0]*e[0]+s[1]*e[1]+s[2]*e[2]),a[2]=t[0],a[6]=t[1],a[10]=t[2],a[14]=-(t[0]*e[0]+t[1]*e[1]+t[2]*e[2]),a[3]=0,a[7]=0,a[11]=0,a[15]=1,a}const qt=`
attribute vec2 aPos;
void main() {
...
`;function Wt(e){const n=e*e,l=1+Math.cbrt(Math.max(1-n,1e-12))*(Math.cbrt(1+e)+Math.cbrt(Math.max(1-e,1e-6))),t=Math.sqrt(3*n+l*l);return(3+t-Math.sqrt(Math.max((3-l)*(3+l+2*t),0)))*.5}function H(e,n,l){if(e==="accretion"){const t=n+Math.random()*(l-n),r=Math.random()*Math.PI*2,s=1/Math.sqrt(t),a=.02*Math.random();return{x:t*Math.cos(r)*(1+a),y:(Math.random()-.5)*.04,z:t*Math.sin(r),vx:-s*Math.sin(r)*.3,vy:0,vz:s*Math.cos(r)*.3,life:0,maxLife:4+Math.random()*6,size:2+Math.random()*3,type:e,cr:1,cg:.7+Math.random()*.3,cb:.2+Math.random()*.3}}else if(e==="jet"){const t=Math.random()>.5?1:-1,r=.08,s=Math.sin(Date.now()*.001)*.03;return{x:(Math.random()-.5)*r+s,y:t*n*.3,z:(Math.random()-.5)*r,vx:(Math.random()-.5)*.04,vy:t*(1.8+Math.random()*.8),vz:(Math.random()-.5)*.04,life:0,maxLife:1.5+Math.random()*2.5,size:1.5+Math.random()*2,type:e,cr:.3+Math.random()*.3,cg:.5+Math.random()*.4,cb:1}}else if(e==="tidal"){const t=Math.random()*Math.PI*2,r=n*(1+Math.random()*.5);return{x:r*Math.cos(t),y:(Math.random()-.5)*.02,z:r*Math.sin(t),vx:-Math.cos(t)*.4,vy:(Math.random()-.5)*.1,vz:-Math.sin(t)*.4,life:0,maxLife:1+Math.random()*1.5,size:1+Math.random()*1.5,type:e,cr:1,cg:.3,cb:.1}}else if(e==="penrose"){const t=Math.random()*Math.PI*2,r=n*1.1,s=Math.random()>.5;return{x:r*Math.cos(t),y:(Math.random()-.5)*.05,z:r*Math.sin(t),vx:(s?1:-1)*Math.cos(t)*.5,vy:s?.3:-.1,vz:(s?1:-1)*Math.sin(t)*.5,life:0,maxLife:s?3:.8,size:s?3:1.5,type:e,cr:s?.2:.8,cg:s?.8:.2,cb:s?1:.3}}else{const t=Math.random()*Math.PI*2,r=l*(1.2+Math.random()*.5),s=.08*(Math.random()-.3);return{x:r*Math.cos(t),y:(Math.random()-.5)*.3,z:r*Math.sin(t),vx:-Math.cos(t)*.15+s*-Math.sin(t),vy:0,vz:-Math.sin(t)*.15+s*Math.cos(t),life:0,maxLife:4+Math.random()*4,size:1.5+Math.random()*2.5,type:e,cr:.9,cg:.5,cb:.2}}}function Z(e,n,l,t){const a=Math.min(.98,.01+(1-Math.min(e/20,1))*(n-.01)),S=Rt(a),h=gt(I[0].c,I[0].w),A=Tt(h);return{omega:a,gamma:S,regime:A.regime+(A.isCritical?" (Critical)":""),redshift:Vt(a),escapeV:kt(a),hawkingT:Dt(h.kappa),distance:e,F:h.F,IC:h.IC,kappa:h.kappa,S:h.S,C:h.C,delta:h.delta,frameDrag:Bt(a,l),penroseEff:_t(l),entropy:Lt(h.kappa),precession:It(a),gwStrainVal:vt(n,t,e),surfGravity:zt(a),timeFactor:Ft(a),radEfficiency:wt(l),spinStar:l}}function Xt(e,n){const l=e.getContext("webgl",{antialias:!1,alpha:!1,premultipliedAlpha:!1,powerPreference:"high-performance"});if(!l)return console.error("WebGL not available"),{destroy:()=>{},getHUD:()=>Z(10,.95,.7,0)};const t=l;t.getExtension("OES_standard_derivatives"),t.enable(t.DEPTH_TEST),t.enable(t.BLEND),t.blendFunc(t.SRC_ALPHA,t.ONE_MINUS_SRC_ALPHA),t.clearColor(0,0,0,1);function r(i,m){const d=t.createShader(i);return t.shaderSource(d,m),t.compileShader(d),t.getShaderParameter(d,t.COMPILE_STATUS)||console.error("Shader:",t.getShaderInfoLog(d)),d}function s(i,m){const d=t.createProgram();return t.attachShader(d,r(t.VERTEX_SHADER,i)),t.attachShader(d,r(t.FRAGMENT_SHADER,m)),t.linkProgram(d),t.getProgramParameter(d,t.LINK_STATUS)||console.error("Link:",t.getProgramInfoLog(d)),d}const a=s(qt,Nt),S=s(Yt,Kt),h=t.createBuffer();t.bindBuffer(t.ARRAY_BUFFER,h),t.bufferData(t.ARRAY_BUFFER,new Float32Array([-1,-1,1,-1,-1,1,1,1]),t.STATIC_DRAW);const A=.95,E=.7,K=I[0].c.length>3?gt(I[0].c,I[0].w).C:0,y=Wt(E),_=y*5.5,xt=800,p=[];for(let i=0;i<450;i++){const m=i<240?"accretion":i<340?"jet":"infalling",d=H(m,y,_);d.life=Math.random()*d.maxLife,p.push(d)}const bt=t.createBuffer();let B=0,F=.35,b=8,P=!1,R=0,T=0,D=!0,k=0,M=.35,z=8;e.addEventListener("mousedown",i=>{P=!0,D=!1,R=i.clientX,T=i.clientY,e.style.cursor="grabbing"}),e.addEventListener("mousemove",i=>{P&&(k+=(i.clientX-R)*.005,M+=(i.clientY-T)*.005,M=Math.max(-Math.PI*.45,Math.min(Math.PI*.45,M)),R=i.clientX,T=i.clientY)}),e.addEventListener("mouseup",()=>{P=!1,e.style.cursor="grab"}),e.addEventListener("mouseleave",()=>{P=!1,e.style.cursor="grab"}),e.addEventListener("touchstart",i=>{P=!0,D=!1,R=i.touches[0].clientX,T=i.touches[0].clientY,i.preventDefault()},{passive:!1}),e.addEventListener("touchmove",i=>{P&&(k+=(i.touches[0].clientX-R)*.005,M+=(i.touches[0].clientY-T)*.005,M=Math.max(-Math.PI*.45,Math.min(Math.PI*.45,M)),R=i.touches[0].clientX,T=i.touches[0].clientY,i.preventDefault())},{passive:!1}),e.addEventListener("touchend",()=>{P=!1}),e.addEventListener("wheel",i=>{z+=i.deltaY*.01,z=Math.max(2,Math.min(25,z)),i.preventDefault()},{passive:!1}),e.addEventListener("dblclick",()=>{D=!0,M=.35,z=8}),e.style.cursor="grab";const u={aPos:t.getAttribLocation(a,"aPos"),uResolution:t.getUniformLocation(a,"uResolution"),uTime:t.getUniformLocation(a,"uTime"),uSpinStar:t.getUniformLocation(a,"uSpinStar"),uCamPos:t.getUniformLocation(a,"uCamPos"),uCamRight:t.getUniformLocation(a,"uCamRight"),uCamUp:t.getUniformLocation(a,"uCamUp"),uCamFwd:t.getUniformLocation(a,"uCamFwd"),uFovTan:t.getUniformLocation(a,"uFovTan"),uGWStrain:t.getUniformLocation(a,"uGWStrain")},g={aPosition:t.getAttribLocation(S,"aPosition"),aColor:t.getAttribLocation(S,"aColor"),aSize:t.getAttribLocation(S,"aSize"),uMVP:t.getUniformLocation(S,"uMVP")},rt=.9;function at(){const i=Math.min(window.devicePixelRatio||1,2.5),m=e.clientWidth,d=e.clientHeight;e.width=Math.round(m*i),e.height=Math.round(d*i),t.viewport(0,0,e.width,e.height)}at();const nt=new ResizeObserver(at);nt.observe(e);let W=Z(b,A,E,K),it=!0,X=0;function st(i){if(!it)return;const m=Math.min((i-X)/1e3,.05);X=i,D&&(k+=m*.08),B+=(k-B)*.05,F+=(M-F)*.05,b+=(z-b)*.05;const d=b*Math.cos(F)*Math.sin(B),ct=b*Math.sin(F),lt=b*Math.cos(F)*Math.cos(B),V=[d,ct,lt],w=N([-d,-ct,-lt]);let mt=[0,1,0];Math.abs(w[1])>.99&&(mt=[0,0,1]);const U=N(Y(w,mt)),j=Y(U,w),Mt=e.width/e.height,Ct=Ot(rt,Mt,.1,100),St=Gt(V,[0,0,0],[0,1,0]),yt=Ht(Ct,St),Pt=i/1e3,At=Math.min(vt(A,K,b),1);t.clear(t.COLOR_BUFFER_BIT|t.DEPTH_BUFFER_BIT),t.depthMask(!1),t.disable(t.DEPTH_TEST),t.useProgram(a),t.bindBuffer(t.ARRAY_BUFFER,h),t.enableVertexAttribArray(u.aPos),t.vertexAttribPointer(u.aPos,2,t.FLOAT,!1,0,0),t.uniform2f(u.uResolution,e.width,e.height),t.uniform1f(u.uTime,Pt),t.uniform1f(u.uSpinStar,E),t.uniform3f(u.uCamPos,V[0],V[1],V[2]),t.uniform3f(u.uCamRight,U[0],U[1],U[2]),t.uniform3f(u.uCamUp,j[0],j[1],j[2]),t.uniform3f(u.uCamFwd,w[0],w[1],w[2]),t.uniform1f(u.uFovTan,1/Math.tan(rt*.5)),t.uniform1f(u.uGWStrain,At),t.drawArrays(t.TRIANGLE_STRIP,0,4),t.disableVertexAttribArray(u.aPos),t.depthMask(!0),t.enable(t.DEPTH_TEST);for(let f=p.length-1;f>=0;f--){const o=p[f];if(o.life+=m,o.life>o.maxLife){p[f]=H(o.type,y,_);continue}const c=Math.sqrt(o.x*o.x+o.z*o.z)||.1;if(o.type==="accretion"){const x=1/(c*Math.sqrt(c))*.5,O=-o.z/c,Et=o.x/c,dt=E*.005/(c*c+.1),ft=.008/(c+.1);if(o.vx=O*(x+dt)-o.x/c*ft,o.vz=Et*(x+dt)-o.z/c*ft,c<y*1.2){const Q=Math.max(.1,(c-.2)/y);o.cr*=Q,o.cg*=Q,o.cb*=Q}}else if(o.type==="jet")o.vy*=1.008,o.vx*=.99,o.vz*=.99;else if(o.type==="tidal"){const x=.15/(c*c+.01);o.vx-=o.x/c*x*m,o.vz-=o.z/c*x*m,o.size*=1.01,o.cr=Math.min(1,o.cr+m*.3),o.cg=Math.min(1,o.cg+m*.2),o.cb=Math.min(1,o.cb+m*.15)}else if(o.type==="penrose")(o.vx*o.x+o.vz*o.z)/c>0?(o.vx*=.998,o.vz*=.998):(o.vx-=o.x/c*.1*m,o.vz-=o.z/c*.1*m);else{const x=.05/(c*c+.01);o.vx-=o.x/c*x*m,o.vz-=o.z/c*x*m;const O=E*.003/(c*c+.1);o.vx+=-o.z/c*O*m,o.vz+=o.x/c*O*m}o.x+=o.vx*m,o.y+=o.vy*m,o.z+=o.vz*m;const v=Math.sqrt(o.x*o.x+o.y*o.y+o.z*o.z);(v<.15||v>20)&&(p[f]=H(o.type,y,_))}for(;p.length<xt;){const f=Math.random(),o=f<.4?"accretion":f<.58?"jet":f<.75?"infalling":f<.88?"tidal":"penrose";p.push(H(o,y,_))}const C=new Float32Array(p.length*7);for(let f=0;f<p.length;f++){const o=p[f],c=Math.min(1,Math.min(o.life/.3,(o.maxLife-o.life)/.5)),v=f*7;C[v]=o.x,C[v+1]=o.y,C[v+2]=o.z,C[v+3]=o.cr*c,C[v+4]=o.cg*c,C[v+5]=o.cb*c,C[v+6]=o.size*(.5+c*.5)}t.useProgram(S),t.bindBuffer(t.ARRAY_BUFFER,bt),t.bufferData(t.ARRAY_BUFFER,C,t.DYNAMIC_DRAW);const J=28;t.enableVertexAttribArray(g.aPosition),t.vertexAttribPointer(g.aPosition,3,t.FLOAT,!1,J,0),t.enableVertexAttribArray(g.aColor),t.vertexAttribPointer(g.aColor,3,t.FLOAT,!1,J,12),t.enableVertexAttribArray(g.aSize),t.vertexAttribPointer(g.aSize,1,t.FLOAT,!1,J,24),t.uniformMatrix4fv(g.uMVP,!1,yt),t.blendFunc(t.SRC_ALPHA,t.ONE),t.drawArrays(t.POINTS,0,p.length),t.disableVertexAttribArray(g.aPosition),t.disableVertexAttribArray(g.aColor),t.disableVertexAttribArray(g.aSize),t.blendFunc(t.SRC_ALPHA,t.ONE_MINUS_SRC_ALPHA),W=Z(b,A,E,K),n&&n(W),requestAnimationFrame(st)}return requestAnimationFrame(i=>{X=i,st(i)}),{destroy:()=>{it=!1,nt.disconnect()},getHUD:()=>W}}const et=document.getElementById("sim-canvas"),jt=document.getElementById("hud-F"),Jt=document.getElementById("hud-omega"),Qt=document.getElementById("hud-IC"),Zt=document.getElementById("hud-kappa"),$t=document.getElementById("hud-S"),te=document.getElementById("hud-delta"),ee=document.getElementById("hud-gamma"),oe=document.getElementById("hud-redshift"),re=document.getElementById("hud-vesc"),ae=document.getElementById("hud-hawking"),ne=document.getElementById("hud-dist"),ie=document.getElementById("hud-spin"),se=document.getElementById("hud-drag"),ce=document.getElementById("hud-penrose"),le=document.getElementById("hud-time"),me=document.getElementById("hud-entropy"),de=document.getElementById("hud-prec"),fe=document.getElementById("hud-gw"),he=document.getElementById("hud-surfg"),ue=document.getElementById("hud-radeff"),ht=document.getElementById("hud-regime"),ut=document.getElementById("regime-bar");let pt=0,$="";const G=document.querySelector("#hud-left .hud-panel"),q=document.querySelector("#hud-right .hud-panel");function pe(e){const n=performance.now();if(n-pt<66)return;pt=n,jt.textContent=e.F.toFixed(4),Jt.textContent=e.omega.toFixed(4),Qt.textContent=e.IC.toFixed(4),Zt.textContent=e.kappa.toFixed(4),$t.textContent=e.S.toFixed(4),te.textContent=e.delta.toFixed(4),ee.textContent=e.gamma.toFixed(3),oe.textContent=e.redshift.toFixed(4),re.textContent=e.escapeV.toFixed(4)+" c",ae.textContent=e.hawkingT.toFixed(6),ne.textContent=e.distance.toFixed(1)+" r_s",ie.textContent=e.spinStar.toFixed(3),se.textContent=e.frameDrag.toFixed(4),ce.textContent=(e.penroseEff*100).toFixed(1)+"%",le.textContent=e.timeFactor.toFixed(4),me.textContent=e.entropy.toFixed(3),de.textContent=e.precession.toFixed(4)+" rad",fe.textContent=e.gwStrainVal.toExponential(2),he.textContent=e.surfGravity.toFixed(4),ue.textContent=(e.radEfficiency*100).toFixed(1)+"%";const l=e.regime.split(" ")[0];ht.textContent=e.regime,ht.className="hud-value regime-"+l.toLowerCase();const r={STABLE:"#059669",WATCH:"#d97706",COLLAPSE:"#dc2626"}[l]||"#333";if(ut.style.backgroundColor=r,ut.style.boxShadow="0 -2px 16px "+r+"80",l!==$&&$!==""){const s=r+"60";G.style.borderColor=r,q.style.borderColor=r,G.style.boxShadow="0 0 20px "+s,q.style.boxShadow="0 0 20px "+s,setTimeout(()=>{G.style.borderColor="",q.style.borderColor="",G.style.boxShadow="",q.style.boxShadow=""},1200)}$=l}Xt(et,pe);const L=document.getElementById("controls-overlay");let ot=setTimeout(()=>L.classList.add("hidden"),6e3);et.addEventListener("mousedown",()=>{clearTimeout(ot),L.classList.add("hidden")});et.addEventListener("touchstart",()=>{clearTimeout(ot),L.classList.add("hidden")});L.addEventListener("click",()=>{clearTimeout(ot),L.classList.add("hidden")});
