...
% ── layout (Nature: single column, double-spaced, line numbers) ──
...
% ── bibliography (Nature: numbered, citation order) ───────────
...
% TITLE
% ============================================================
\title{A single algebraic diagnostic detects structurally identical
phase transitions from quarks to the cosmic horizon}

\author{Cl\'{e}ment Paulus\thanks{Correspondence: clementpaulus9@gmail.com}}
...
% KEYWORDS
% ============================================================
% (Nature Communications: 3–6 terms, placed after abstract)
\newcommand{\keywords}{Phase transitions, cross-scale universality,
quark confinement, information geometry, coherence, computational
verification}
...
% ============================================================
\begin{abstract}
The ratio of composite integrity to fidelity ($\IC/F$)---the
geometric mean of normalized measurements divided by their
arithmetic mean---computed from a single algebraic kernel,
exhibits structurally identical phase-transition signatures
across systems spanning 61~orders of magnitude---from quark
confinement ($10^{-15}$\,m) to dark-energy--dominated cosmology
($10^{26}$\,m). At the quark--hadron
boundary, $\IC/F$ drops by a factor of~23 as color confinement
drives a single measurement channel to zero; atomic structure
subsequently restores coherence 35-fold through emergent degrees of
freedom; dark energy drives the identical collapse mechanism at
$10^{26}$\,m scales. The heterogeneity gap $\Delta = F - \IC$ spikes
at every phase boundary tested, providing a single-number,
dynamics-independent diagnostic for structural fragility.

These patterns emerge from a six-output kernel function
$K\!:\![0,1]^n \times \Delta^n \to (F, \omega, S, C,
\kappa, \IC)$, derived from a single admissibility axiom. The kernel
maps any bounded measurement vector to fidelity~$F$ (weighted
arithmetic mean), log-integrity~$\kappa$ (weighted log-mean), and
composite integrity~$\IC = \exp(\kappa)$ (weighted geometric mean),
among other quantities. Three algebraic identities---$F + \omega
\equiv 1$ (duality), $\IC \leq F$ (integrity bound), and
$\IC = \exp(\kappa)$ (log-integrity relation)---guarantee the
diagnostic's structural reliability across all domains. The integrity
bound $\IC \leq F$ corresponds to the classical weighted inequality
of arithmetic and geometric means; the contribution lies not in the
bound itself but in its systematic application as a scale-independent
phase-transition diagnostic across 20~scientific domains. Exhaustive
computational verification---15{,}260~automated proofs across
406~physical objects---confirms all three identities to machine
precision ($<\!10^{-15}$) with zero violations.
\end{abstract}
...
requiring dynamical input or scale-dependent running. What has
been missing is not cross-scale structure \emph{per se} but a single
algebraic diagnostic---one number, computed identically at every
scale---that detects phase transitions without knowledge of the
underlying dynamics.
...
\item \textbf{Single axiom.} All structure derives from one
  ...
  real}~(Axiom-0). This is not a dynamical law but an
  \emph{admissibility criterion}: only claims that survive a
  collapse--recovery cycle receive epistemic credit. Operationally,
  this produces a composable verification boundary (the \emph{seam}),
  a measured re-entry delay ($\tR$), and three-valued verdicts
  (Conformant / Nonconformant / Non-Evaluable) that replace
  Boolean pass/fail testing.
...
learning, or phenomenological modeling. The identities are
...
Critically, the framework is not a single result but a complete
measurement architecture. The kernel is implemented by an open
protocol and extended via modular domain closures: each closure
declares which real-world quantities become the trace
vector~$\tvec$, frozen in a versioned contract \emph{before}
evaluation. Classical results---including Shannon entropy, the
arithmetic--geometric mean inequality, and the
fluctuation--dissipation theorem---emerge as \emph{degenerate
limits} of the kernel when structural degrees of freedom are
removed (see Discussion), ensuring full compatibility with
established methods.
...
weighted geometric mean. The geometric mean is the natural choice
for integrity because it is the unique mean that decomposes
multiplicatively across subsystems
($\IC_{12} = \sqrt{\IC_1 \cdot \IC_2}$; cf.\ the Bhattacharyya
coefficient) and because it arises as the solvability condition
for trace recovery (see Identity~\ref{id:bound}). The entropy~$S$
is the Bernoulli field entropy---the unique entropy functional for
independent binary channels. The curvature~$C \in [0,1]$ measures
channel heterogeneity normalized to the maximum standard deviation.
...
The three identities rest on classical mathematics;
the contribution is the \emph{empirical discovery} that applying them
as a unified measurement lens across 406~objects reveals
cross-scale structure---the confinement cliff, atomic recovery,
cosmological decoherence---invisible to any single-domain analysis.
The identities provide the \emph{lens}; the contribution is what
the lens reveals.
...
arithmetic and geometric means~\cite{hardylittlewoodpolya1952};
the novelty lies not in the bound itself but in its application as a
scale-independent phase-transition diagnostic.
Three properties give it cross-scale power beyond the classical
statement. First, the bound acquires a \emph{solvability
interpretation}: given only the aggregate values $F$ and $\IC$,
recovering individual channel values in the two-channel case
requires solving $c_{1,2} = F \pm \sqrt{F^2 - \IC^2}$, which
yields real solutions only when $\IC \leq F$. Second, the gap $\Delta = F - \IC$
(the heterogeneity gap) serves as a \emph{scale-independent
phase-transition diagnostic}---it spikes at every boundary where
channels die, irrespective of the underlying physics
(color confinement, chemical inertness, dark energy).
Third, the bound carries \emph{composition laws} absent
from the classical inequality: $\IC$ composes geometrically across
subsystems ($\IC_{12} = \sqrt{\IC_1 \cdot \IC_2}$) while $F$
composes arithmetically ($F_{12} = (F_1 + F_2)/2$), making
cross-scale comparison algebraically rigorous.
...
algebraic: for $n$ independent Bernoulli channels, the central limit
theorem constrains the shape of the channel distribution, making
entropy asymptotically a function of mean ($F$) and spread ($C$).
Across 406~physical objects,
$|\mathrm{corr}(C, S)| > 0.97$ for $n \geq 8$ channels
($N = 406$, $p < 10^{-100}$), approaching~$-1$ in the
large-$n$ limit. This
constraint reduces the six kernel outputs to \textbf{three effective
degrees of freedom} $(F, \kappa, C)$: principal component analysis
over 5~kernel diagnostics across all domains yields 4~effective
dimensions (explaining $> 99\%$ of variance), further reduced to~3
by the algebraic identities.

\subsection{Exhaustive verification}

These identities were verified exhaustively across
...
\item \textbf{Periodic table:} 354~identity checks (three per
  element, all 118), all satisfied to $< 10^{-9}$.
\item \textbf{Standard Model:} 31~particles (17~fundamental +
  14~composite), duality verified to zero
  residual~\cite{pdg2024}.
\item \textbf{Algebraic closure.} Chaining consecutive kernel
  evaluations defines a binary operation on kernel states.
  This \emph{seam composition} is associative to
  $5.55 \times 10^{-17}$ (machine precision), with an identity
  element (the homogeneous trace), forming a
  monoid under chaining. Associativity ensures that nested
  cross-scale evaluations (e.g., quarks $\to$ hadrons $\to$ atoms)
  yield the same result regardless of grouping.
\end{itemize}

In total, 15{,}260~deterministic automated proofs span
20~domains (Table~\ref{tab:domains}), supported by
217~closure modules, 47~proven lemmas, and 44~structural
identities verified to machine precision. To our knowledge,
this represents the most extensive cross-domain verification
of mathematical identities linking subatomic to cosmological
scales.
...
Each domain contributes closures (channel selections and
entity catalogs) validated against the universal kernel.}
\label{tab:domains}
\begin{tabular}{llll}
\toprule
...
\emph{confinement cliff}: a 23-fold discontinuity in $\IC/F$
at the quark--hadron phase boundary (Fig.~\ref{fig:cliff}).

Individual quarks, characterized by their quantum numbers, carry
finite values in all eight channels (mass, spin, charge, color,
weak isospin, lepton number, baryon number, generation). When
quarks bind into color-neutral hadrons~\cite{wilson1974confinement},
the color-charge channel collapses to the guard band
...
\item Proton: $\IC/F = 0.037$ (color $\to \eps$: confinement)
\item Neutron: $\IC/F = 0.0089$ (color + charge $\to \eps$)
\item $\pi^0$: $\IC/F = 0.002$ (neutral, light: multiple
  dead channels)
\end{itemize}

This signature is detectable purely from quantum
numbers~\cite{pdg2024} (mass, spin, charge, color, isospin,
lepton/baryon number, generation), without reference to
QCD coupling constants, lattice calculations~\cite{aoki2006lattice},
or confinement criteria. No existing single-domain diagnostic
connects this hadronic pattern to the identical $\IC/F$ collapse
observed at the noble-gas scale ($Z = 2, 10, 18, \ldots$; see
Fig.~\ref{fig:periodic}) or the dark-energy scale (see
\S\ref{sec:cosmo}). The kernel provides a \emph{scale-independent
diagnostic language} for a phenomenon---channel death destroying
multiplicative coherence---that QCD, atomic physics, and cosmology
each describe separately and in incommensurable terms.

\subsection{Atomic coherence recovery}

Equally remarkable is the 35-fold \emph{recovery} of $\IC/F$ at
...
These emergent channels are well-populated for most elements (no
dead channels except noble gases), correlated through shared
electronic structure, and measured across all 118~elements
(Fig.~\ref{fig:periodic}).

The recovery is not uniform. Period~1 (H, He) remains
near-hadron level ($\IC/F = 0.056$) because hydrogen has
insufficiently populated channels and helium has a near-dead
reactivity channel (the same geometric slaughter mechanism as
confinement, here producing chemical inertness). By Period~4--5, the 8-channel trace
...
This illustrates a structural principle visible across scales:
\emph{rupture at one scale seeds structure at the next}.
The near-zero $\IC$ produced by confinement becomes the boundary
condition for atomic-scale channel formation: color neutrality
and charge stability enable the emergence of new observables
(electronegativity, ionization energy, etc.) that are well-populated,
and the subsequent recovery to $\IC/F > 0.78$ is measurable and
reproducible across all 118~elements.

\subsection{Consciousness coherence}

To test universality beyond fundamental physics, we applied the
same kernel (same equations, same frozen parameters, same
verification protocol) to a 10-channel cortical trace
...
capability but because \emph{all ten channels attain their most
uniform distribution at age~15}, minimizing the heterogeneity gap
$\Delta = F - \IC$. Adult specialization trades uniform coherence
...
experience, flow, anesthesia, coma).
...
present epoch ($z = 0$), $\IC/F$ drops by a factor of~27 (from
0.383 to 0.014) while fidelity~$F$ \emph{rises} (from 0.583 to
0.649).

The mechanism is structurally identical to quark confinement:
when dark energy dominates, the matter-density and
radiation-density channels in the cosmological trace vector
approach~$\eps$, suppressing the geometric mean while preserving
the arithmetic mean. The same mathematical
...
confinement cliff (fundamental $\to$ composite: 23-fold drop,
red arrow) and atomic healing (composite $\to$ atoms: 35-fold
...
red circles) collapses super-exponentially. A single dead channel
in an 8-channel system reduces $\IC/F$ from 1.0 to
0.114---an order-of-magnitude loss from one channel death. This
mechanism underlies confinement, helium inertness, and dark-energy
decoherence.}
\label{fig:slaughter}
\end{figure}
...
Noble gases (red labels) appear as coherence minima owing to
...
parameters produces all data points. The dashed line is a guide to
the eye.}
...
gates partitions the Fisher parameter space into
Stable (12.5\%), Watch (24.4\%), and Collapse (63.1\%),
computed by uniform sampling of $10^4$ trace vectors
$c_i \in [\eps, 1{-}\eps]^n$ and classifying each by the
four-gate criterion. This carries an implication: \emph{stability
is structurally rare} in the admissible parameter space. The
Stable regime requires the conjunction of four conditions
($\omega < 0.038$, $F > 0.90$, $S < 0.15$, $C < 0.14$), and only
12.5\% of the admissible parameter space satisfies all four
simultaneously. While the physical distribution of systems need not
be uniform in this space, the result indicates that stability
requires \emph{simultaneous} satisfaction of multiple conditions, a
constraint consistent with the observation that ordered structure
occupies a narrow region of the available phase space.
...
\item \textbf{Confinement signature in lattice QCD.}
  Lattice simulations that resolve per-flavor condensates
  across the deconfinement transition should exhibit a sharp
  drop in the kernel's $\IC/F$ ratio at the transition
  temperature, consistent with the 23-fold cliff predicted
  from quantum numbers alone.
...
A natural concern is whether the kernel constitutes a closed
formalism incompatible with standard methods. On the contrary,
classical results emerge as \emph{degenerate limits} of the
kernel when structural degrees of freedom are removed. The arrow
of derivation runs from the kernel outward:

\begin{itemize}[leftmargin=1.5em,itemsep=1pt]
\item \textbf{Integrity bound $\to$ AM-GM inequality.}
  $\IC \leq F$ holds for arbitrary heterogeneous traces with
  channel semantics, weights, guard band, and composition laws
  ($\IC$ geometric, $F$ arithmetic). Removing these structural
  elements yields the classical inequality of
  means~\cite{hardylittlewoodpolya1952}.
\item \textbf{Bernoulli field entropy $\to$ Shannon entropy.}
  The full entropy $S = -\sum w_i[c_i \ln c_i + (1{-}c_i)\ln(1{-}c_i)]$
  reduces to Shannon $H = -\sum p_i \ln p_i$ when the collapse
  field is removed (i.e., $c_i \in \{0,1\}$
  only)~\cite{shannon1948}.
\item \textbf{Duality identity $\to$ completeness constraints.}
  $F + \omega \equiv 1$ exemplifies the general structure
  ``preserved + lost = total.'' The same algebraic structure
  appears in fluctuation--dissipation relations (signal +
  dissipation = input) and unitarity ($|\alpha|^2 + |\beta|^2 = 1$
  for quantum amplitudes). GCD does not derive these physical
  theorems; rather, the duality identity shares their algebraic
  skeleton, suggesting a common structural origin.
\item \textbf{Cost function $\to$ critical slowing.}
  $\Gamma(\omega) = \omega^3/(1{-}\omega)$ generates a full phase
  diagram; restricting to the neighborhood of the transition
  point yields behavior consistent with critical slowing
  from renormalization group theory~\cite{gross1973}.
\end{itemize}

\noindent Each classical result is therefore a \emph{special case}
obtained by removing structure, not an external theory that the
kernel must be reconciled with.

This approach is complementary to, not competitive with, existing
cross-scale methods. Renormalization group flows describe how
coupling constants evolve with energy scale and require dynamical
Lagrangians as input; the kernel operates on static measurements
and requires no dynamics. Fisher information
geometry~\cite{amari2016information} characterizes the Riemannian
structure of statistical manifolds; the kernel quantifies
\emph{multiplicative coherence} of physical channels through a
single number ($\IC/F$) that is algebraically exact, not
asymptotically valid. The contribution is empirical: no
existing cross-scale diagnostic has been shown to yield the
\emph{same} phase-transition signature at both the hadron and
cosmological scales.

\subsection{Cross-domain translation protocol}

Cross-scale invariants are only useful if domain specialists
can interpret them. The framework includes a built-in translation
protocol (the ``Rosetta'') that maps the kernel's five diagnostic
words---\emph{Drift} ($\omega$), \emph{Fidelity} ($F$),
\emph{Roughness} ($C$), \emph{Return} ($\tR$), and
\emph{Integrity} ($\IC$)---into nine disciplinary lenses:
epistemology, ontology, phenomenology, history, policy, physics,
finance, security, and semiotics. The operational meaning (tied
to the kernel and its ledger) is invariant across lenses; only
the narrative dialect changes.

For example, the confinement cliff ($\IC/F$: $0.506 \to 0.022$)
is described in the physics lens as ``coupling to uncontrolled
degrees of freedom destroying multiplicative coherence,'' in the
finance lens as ``single-channel concentration risk collapsing
portfolio integrity,'' and in the semiotics lens as ``loss of
referential grounding breaking the chain of interpretation.''
All three descriptions refer to the same invariant, the same
number, and the same ledger entry---they differ only in vocabulary.

This translation layer is itself formalized as one of the
20~domain closures: the \emph{dynamic semiotics} closure maps
30~sign systems (natural languages, formal notations, genetic
codes, animal communication) to an 8-channel semiotic kernel
(sign repertoire, interpretant depth, ground stability,
translation fidelity, semiotic density, indexical coupling,
iconic persistence, symbolic recursion). All three algebraic
identities hold for every sign system tested, and the analysis
reveals that meaning coherence is governed by
\emph{density~$\times$~depth}, not
\emph{stability~$\times$~resemblance}---a result
with implications for scientific communication itself. The
framework can therefore analyze its own communicability as a
domain closure, a self-referential consistency check unavailable
to standard measurement theories.

\subsection{Implications for measurement theory}

The framework constitutes what we term a \emph{cognitive
equalizer}---a measurement system that eliminates
analyst-dependent variance: because all thresholds are frozen
(not chosen by the analyst), all vocabulary is operationally
defined, and all verdicts are three-valued (Conformant /
Nonconformant / Non-Evaluable), two independent analysts
processing the same data under the same contract must arrive
at the same verdict. This removes agent-dependent variance
from measurement interpretation---a property related to the
information-geometric program of Amari~\cite{amari2016information}
...
limitation but the basis of its reliability. Any measurement
framework that produces ``correct'' outputs regardless of input
quality is not measuring anything---it is fitting. The kernel's
strict input--output contract enforces the opposite: accurate
inputs yield accurate invariants; inaccurate inputs yield
...
comparisons) is only as good as the channel selection.

That the kernel faithfully propagates input quality, rather than
absorbing noise into flexible parameters, is what distinguishes it
as a measurement instrument rather than a fitting procedure. The
...
Two scope limitations deserve explicit statement. First, the
...
primary neuroimaging---an area where independent replication with
raw data would be particularly valuable. Second, the kernel is
static: it maps a trace vector to invariants at a single
evaluation, without dynamical evolution equations. Extending the
framework to time-dependent processes is a direction for future
work~\cite{paulus2025umcp}.
...
normalization functions. Importantly, the choice of which
observables become channels is \emph{not hidden}: it is the declared
empirical content of each application, specified in a versioned
contract and frozen before evaluation. Different channel selections
produce different $\IC/F$ profiles---by design, since this is the
measurement---but the three algebraic identities hold for
\emph{every} admissible channel selection, guaranteeing that the
framework's structural properties are independent of the
researcher's modeling choices. Standard Model particles use $n = 8$~channels
(mass$_{\log}$, spin normalized to $[0,1]$,
$|Q|/\max|Q|$, color (1 if non-singlet, $\eps$ otherwise),
...
(ionization energy, electronegativity, atomic radius, density,
melting point, boiling point, electron affinity, covalent radius),
each mapped affinely to $[0, 1]$ using the empirical minimum and
maximum across all 118~elements. Consciousness systems use $n = 10$
cortical channels (prefrontal, temporal, parietal, occipital, motor,
sensory, limbic, cerebellar, brainstem, thalamic coherence),
with values estimated from published fMRI and EEG meta-analyses and
normalized to $[0, 1]$ by the empirical range across 20~systems.
Every normalization function is
...
All 15{,}260~tests run under Python~3.12 with NumPy~1.26 using
...
repository~\cite{umcpmetadatarepo} under the MIT license. The
kernel implementation is in
\texttt{src/umcp/kernel\_optimized.py}. SHA-256 integrity checksums
for 228~tracked files are maintained in \texttt{integrity/sha256.txt}
