...
cross-scale universality; geometric slaughter; memory wells;
...
Section~\ref{sec:biology} shows the pattern is predictive;
Section~\ref{sec:memory} derives clinical consequences from
...
the horizon.  The photon and the black hole are structural
kin: both suffer from the impossibility of closed return, and
both are classified as \emph{thin arc}
...
At the biological scale, the question of why life is
carbon-based has a structural answer:
...
hole (0.121), proton (0.134), electron (0.141), photon (0.245).
No entity in this catalog reaches the distorted threshold.
(4/4 sub-tests.)
\end{remark}
...
Key (Sec.~\ref{sec:key}) predicts what the data will show:
...
prediction before seeing the table.  The structural
prediction matched the data --- the pattern generalizes
independently of domain expertise.  The same diagnostic
intuition is powerful enough to derive a clinical diagnosis
from first principles (Sec.~\ref{sec:memory}).
...
\textbf{Derive the signature before seeing the data.}
The Diagnostic Key provides everything needed.  Construct
the PTSD prediction:
\begin{enumerate}
...
\item The structural parallel is to confined quarks inside
  ...
  Table~\ref{tab:subatomic}): predict $\IC/F \approx
  0.72$--$0.76$.
\end{enumerate}
...
geometry (lensing and gravity), prediction (biology confirmed
structural expectations), and now \emph{consequence} ---
the mathematics tells a clinician which channels are dead
and in what order they should be restored.  Each register
...
the full entity catalog (37~entities, 8~categories), the
...
runs 129~tests in $< 300$~ms with zero failures.
...
consequence --- is the enrichment that linear reading adds.
