...
%  Format: RevTeX4-2, prx, twocolumn
%  Figures: 4
%  References: ~50
%
...
% TITLE
% ============================================================
\title{Cross-Scale Integrity Diagnostics: Algebraic Invariants\\
and Universal Phase-Transition Signatures Spanning\\
...
% ============================================================
\begin{abstract}
The ratio of geometric to arithmetic coherence ($\IC/F$), computed
from a single algebraic kernel applied to normalised physical
measurements, exhibits structurally identical phase-transition
signatures across systems spanning 61~orders of magnitude---from
quark confinement ($10^{-15}$\,m) to dark-energy--dominated cosmology
($10^{26}$\,m). At the quark--hadron boundary, $\IC/F$ collapses
$23\times$ as colour confinement drives a single measurement channel
to zero; atomic structure subsequently restores coherence
$35\times$ through emergent degrees of freedom; dark energy
reproduces the identical collapse mechanism at $10^{26}$\,m scales.
The heterogeneity gap $\Delta = F - \IC$ spikes at every phase
boundary tested, providing a single-number diagnostic for structural
fragility independent of the underlying dynamics.

These patterns emerge from a six-output kernel function
$K\!:\![0,1]^n \times \Delta^n \to (F, \omega, S, C, \kappa, \IC)$,
derived from a single admissibility axiom, that maps any bounded
measurement vector to fidelity~$F$ (weighted arithmetic mean),
log-integrity~$\kappa$ (weighted log-mean), and composite
integrity~$\IC = \exp(\kappa)$ (weighted geometric mean), among
other quantities. Three algebraic identities---$F + \omega \equiv 1$
(duality), $\IC \leq F$ (integrity bound), and
$\IC = \exp(\kappa)$ (log-integrity relation)---guarantee the
diagnostic's structural reliability across all domains.
The integrity bound $\IC \leq F$ instantiates the classical weighted
inequality of arithmetic and geometric means; the contribution is
not the bound itself but its systematic application as a
scale-independent phase-transition diagnostic across 20~scientific
domains. Exhaustive computational verification---11{,}342~automated
proofs across 406~physical objects---confirms all three identities
to machine precision ($<\!10^{-15}$) with zero violations.
\end{abstract}
...
of which require dynamical input or scale-dependent running. What has
...
\item \textbf{Single axiom.} All structure derives from one
  ...
  this produces the seam calculus (a composable verification
  boundary between outbound collapse and demonstrated return), the
  return time~$\tR$ (a measured re-entry delay, not an assumed one),
  and three-valued verdicts (Conformant / Nonconformant /
  Non-Evaluable) that replace the Boolean pass/fail of conventional
  testing. Without Axiom-0 one has a kernel function; with it, one
  has a measurement protocol that distinguishes verified claims
  (\emph{welds}) from internally consistent but non-returning
  assertions (\emph{gestures}).
...
measurement architecture. The kernel (Tier-1) is implemented by
an open protocol (Tier-0) and extended via modular domain
closures (Tier-2): each closure declares which real-world
quantities become the trace vector~$\tvec$, frozen in a versioned
contract \emph{before} evaluation. The system currently comprises
217~closure modules, 47~proven lemmas, 44~structural identities,
21~versioned contracts, and 28~validated casepacks. Classical
results---including Shannon entropy, the arithmetic--geometric
mean inequality, and the fluctuation--dissipation
theorem---emerge as \emph{degenerate limits} of the kernel when
structural degrees of freedom are removed (see Discussion),
ensuring full compatibility with established methods. A built-in
cross-domain translation protocol (the ``Rosetta'') maps the
kernel's five-word diagnostic vocabulary into nine disciplinary
lenses, enabling physicists, neuroscientists, and economists to
read the same invariants in their native dialect while the
underlying audit remains invariant.
...
coefficient), making it the correct aggregator for composition
across scales, and because it emerges as the solvability condition
...
channel heterogeneity normalised to the maximum standard deviation.
...
That three identities grounded in classical mathematics can serve as
a cross-scale diagnostic is precisely the point: the mathematics is
not novel; the \emph{empirical discovery} is that applying these
identities as a unified measurement lens across 406~objects reveals
...
$\IC \leq F$ is the classical weighted inequality of arithmetic and
geometric means~\cite{hardylittlewoodpolya1952}; the novelty lies
not in the bound itself but in its application as a
...
recovering individual channel values requires solving
$c_{1,2} = F \pm \sqrt{F^2 - \IC^2}$, which yields real solutions
only when $\IC \leq F$. Second, the gap $\Delta = F - \IC$
(the heterogeneity gap) serves as a \emph{scale-independent
phase-transition diagnostic}---it spikes at every boundary where
channels die, regardless of the physics producing the death
(colour confinement, chemical inertness, dark energy).
...
theorem constrains the channel distribution's shape, making entropy
asymptotically a function of mean ($F$) and spread ($C$).
Across 406~physical objects,
the Pearson correlation
$|\mathrm{corr}(C, S)| > 0.97$ for $n \geq 8$ channels
($N = 406$, $p < 10^{-100}$), and approaches~$-1$ in the
large-$n$ limit. This
...
\item \textbf{Algebraic closure.} When two consecutive kernel
  evaluations are chained---the output of evaluating system~$A$
  feeds as context into evaluating system~$B$---the composition
  defines a binary operation on kernel states. This \emph{seam
  composition} is associative to
  $5.55 \times 10^{-17}$ (machine precision; IEEE~754
  floating-point arithmetic is \emph{not} generally associative,
  so this result confirms algebraic exactness beyond numerical
  accident), with an identity
  element (the homogeneous trace), proving the framework forms a
  monoid under chaining. Associativity guarantees that nested
...
this constitutes the most extensive cross-domain verification
...
ionisation energy, atomic radius, melting point, boiling point,
...
condition for atomic-scale channel formation: colour neutrality
and charge stability enable the emergence of new observables
(electronegativity, ionisation energy, etc.) that are well-populated,
...
Fisher parameter space (computed by uniform sampling of $10^4$
trace vectors $c_i \in [\eps, 1{-}\eps]^n$ and classifying each
by the four-gate criterion)---carries an implication for fundamental
...
\item \textbf{Confinement signature in lattice QCD.}
  Lattice simulations that resolve per-flavour condensates
  across the deconfinement transition should exhibit a sharp
  drop in the kernel's $\IC/F$ ratio at the transition
  temperature, consistent with the $23\times$ cliff predicted
...
formalism incompatible with standard methods. The opposite is the
case: classical results emerge as \emph{degenerate limits} of the
...
\item \textbf{Integrity bound $\to$ AM-GM inequality.}
  ...
  ($\IC$ geometric, $F$ arithmetic). Stripping all structure
  recovers the classical inequality of
...
\item \textbf{Duality identity $\to$ completeness constraints.}
  $F + \omega \equiv 1$ instantiates the general structure
  ``preserved + lost = total.'' This structure appears in
  fluctuation--dissipation relations (signal + dissipation = input)
  and unitarity ($|\alpha|^2 + |\beta|^2 = 1$ for quantum
  amplitudes). GCD does not derive these physical theorems;
  rather, the duality identity shares their algebraic skeleton,
  suggesting a common structural origin.
\item \textbf{Cost function $\to$ critical slowing.}
  $\Gamma(\omega) = \omega^3/(1{-}\omega)$ generates a full phase
  diagram; restricting to the neighbourhood of the transition
  point recovers critical exponents from renormalization group
  theory~\cite{gross1973}.
\end{itemize}
...
geometry~\cite{amari2016information} characterises the Riemannian
...
portfolio integrity,'' and in the semiotics lens as ``dead
indexical coupling killing sign-chain closure.'' All three
descriptions refer to the same invariant, the same number, and
the same ledger entry---they differ only in vocabulary.

This translation layer is itself formalised as one of the
...
framework can therefore analyse its own communicability as a
Tier-2 closure, a self-referential consistency check unavailable
...
comparisons) is only as good as the channel selection. That the
kernel faithfully propagates input quality, rather than absorbing
noise into flexible parameters, is precisely what makes it a
measurement instrument rather than a fitting procedure. The Tier-2
...
normalisation functions. Importantly, the choice of which
...
researcher's modelling choices. Standard Model particles use $n = 8$~channels
...
normalised to $[0, 1]$ by the empirical range across 20~systems.
Every normalisation function is
