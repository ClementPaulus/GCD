...
\begin{tabular}{@{}c@{\,}l@{\quad}l@{}}
  \toprule
  \textbf{Stop} & \textbf{What happens} & \textbf{Spine stage} \\
  \midrule
  1 & Axiom declared    & \multirow{2}{*}{\textsc{Contract}} \\
  2 & Parameters frozen  & \\
  \midrule
  3 & Channels chosen    & \multirow{3}{*}{\textsc{Canon}} \\
  4 & Trace bounded      & \\
  5 & Kernel computed    & \\
  \midrule
  6 & Ledger debited / credited & \textsc{Closures} \\
  \midrule
  7 & Regime classified  & \textsc{Integrity Ledger} \\
  \midrule
  8 & Casepack assembled & \multirow{5}{*}{\textsc{Stance}} \\
  9 & CLI executed       & \\
  10 & API called        & \\
  11 & Latin rendered    & \\
  12 & Ordinary reading  & \\
...
domains~\cite{paulus2025physicscoherence,umcpmetadatarepo}.  The
exponent $p = 3$ is the unique integer for which
$\omega_{\mathrm{trap}}$ is a Cardano root of $x^3 + x - 1 = 0$.
The tolerance $\tolseam = 0.005$ is the width at which $\IC \leq F$
holds at 100\% across every domain.  The guard band $\eps = 10^{-8}$
is chosen so that the pole at $\omega = 1$ does not affect any
measurement to machine precision.
...
casepack is a directory containing the raw data, the frozen contract,
the closure registry, and the expected outputs (embedded trace,
invariants, SS1M receipt with SHA-256 checksums).
...
assert result["run_status"] == "CONFORMANT"
\end{Verbatim}
...
\subsection{The cognitive equalizer}
