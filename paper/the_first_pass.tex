...
\begin{abstract}
We trace a single measurement---three voltage readings at 99\%
fidelity---through the complete validation spine of the Universal
Measurement Contract Protocol (UMCP).  Starting from Axiom-0
(``Collapse is generative; only what returns is real'') and a frozen
contract, we embed the raw data into a bounded trace vector
$\tvec \in [\eps, 1-\eps]^3$, compute the six kernel invariants
$(F, \omega, S, C, \kappa, \IC)$, reconcile the integrity ledger,
classify the regime via four frozen gates, and derive the three-valued
verdict \conform{}.  Every number is exact, every identity is verified
to machine precision, and every step is reproducible from the
open-source reference implementation.  We present the kernel
computation in explicit detail, prove the three structural identities
($F + \omega = 1$, $\IC \leq F$, $\IC = e^\kappa$) for the worked
example, demonstrate the seam budget closure, and show how the same
spine applies identically regardless of the interface used---command
line, Python API, or REST endpoint.  A heterogeneous counterexample
is provided to demonstrate geometric slaughter: the mechanism by which
a single degraded channel destroys multiplicative coherence while
leaving the arithmetic mean apparently healthy.  The paper serves as
a self-contained entrance to the GCD formalism, requiring no prior
...
(UMCP)~\cite{paulus2025umcp,paulus2025physicscoherence}, constitute a
contract-first validation system built on a single axiom.  The
framework has been applied across 23 domain
closures~\cite{umcpmetadatarepo}, from particle
physics~\cite{paulus2025physicscoherence} to consciousness coherence,
with 20{,}221 automated tests, 47~lemmas, and 44~structural
identities verified to machine precision.  A \emph{closure} in this
context is a domain-specific mapping layer: it selects which
...
Despite this scope, no single document has previously traced one
complete object through every layer of the system: from the axiom
through the frozen contract, embedding, kernel computation, ledger
reconciliation, regime classification, casepack packaging, and
operational execution---all with exact numbers.

This paper provides that trace.  The object is deliberately minimal:
three identical voltage readings.  The simplicity is the point.  When
the system is understood for the simplest case, the general case
follows by replacing the trace vector---the spine does not change.

The choice of a trivial example is architectural, not pedagogical
laziness.  The validation spine is \emph{invariant} across domains:
the same frozen contract, kernel, ledger, regime gates, and verdict
logic apply whether the trace vector carries three voltages or
thirty-one Standard Model particles.  Only the Tier-2 closure
changes---the mapping from raw observables to the unit interval.
One complete pass through the simplest case therefore constitutes
the correct entrance to the entire framework, because a reader who
has verified the spine for three homogeneous channels can then
navigate any domain closure by understanding only the channel
selection, never the protocol itself.
...
\item \textbf{Integrity Ledger} (Stops~6--7): The seam residual is
  checked; the regime is classified.
\item \textbf{Stance} (Stops~7--12): The verdict is derived;
...
\begin{tabular}{@{}c@{\,}l@{\quad}l@{\quad}l@{}}
  \toprule
  \textbf{Stop} & \textbf{What happens} & \textbf{Spine stage} & \textbf{Tier} \\
  \midrule
  1 & Axiom declared    & \multirow{2}{*}{\textsc{Contract}} & Tier-1 \\
  2 & Parameters frozen  & & Tier-0 \\
  \midrule
  3 & Channels chosen    & \multirow{3}{*}{\textsc{Canon}} & Tier-2 \\
  4 & Trace bounded      & & Tier-0 \\
  5 & Kernel computed    & & $1 \!\leftarrow\! 0$ \\
  \midrule
  6 & Ledger debited / credited & \textsc{Closures} & Tier-0 \\
  \midrule
  7 & Regime classified  & \textsc{Integrity Ledger} & Tier-0 \\
  \midrule
  8 & Casepack assembled & \multirow{5}{*}{\textsc{Stance}} & Tier-0 \\
  9 & CLI executed       & & Tier-0 \\
  10 & API called        & & Tier-0 \\
  11 & Latin rendered    & & Tier-2 \\
  12 & Ordinary reading  & & Tier-2 \\
  \bottomrule
\end{tabular}
\caption{The twelve stops mapped onto the five stages of the
  fixed discourse spine:
  Contract~$\to$~Canon~$\to$~Closures~$\to$~Integrity
  Ledger~$\to$~Stance.  The \textbf{Tier} column shows whether each
  stop is Tier-1 (the kernel and its identities), Tier-0 (the protocol
  that implements and interprets the kernel), or Tier-2 (domain-specific
  channel selection).  ``$1 \!\leftarrow\! 0$'' denotes a crossing:
  Tier-0 code evaluates a Tier-1 function.}
\label{fig:spine}
\end{figure}
...
\begin{ruledtabular}
\begin{tabular}{P{1.3cm}P{1.8cm}P{1.9cm}P{1.5cm}}
  Object & Computed from & Meaning & This example \\
  \hline
  Regime & Four gates on $(F,\omega,S,C)$ & Structural phase of the
    trace & \regime{Stable} \\
  Stance / verdict & Ledger reconciliation + regime + identity checks
    & Auditable three-valued judgment & \conform{} \\
  Validator result & Schema, semantics, checksums, identities & Protocol
    compliance of the casepack & \conform{}, 0 errors \\
  Return status & $\tR$ from prior history & Whether the system has
    re-entered $D_\theta$ & $\tR = \IR$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\begin{Verbatim}[fontsize=\small]
from umcp.measurement_engine \
    import MeasurementEngine
import numpy as np
...
The following section is not ornamentation.  The GCD framework
maintains a canonical Latin discourse layer because each Latin term
carries morphological constraints that English prose does not---the
operational definition is encoded in the word's structure.  This
section demonstrates that the worked example can be rendered in full
canonical form, verifying that the five-word vocabulary and the
Rosetta translation are operational, not decorative.

The canonical Latin vocabulary carries operational constraints that
English does not.  Each term is not a translation but a precision
instrument whose morphology encodes the operational definition.
...
We have traced three voltage readings---the simplest possible
measurement---through the complete GCD/UMCP spine.  The trace
demonstrates:
...
\item \textbf{Typed boundaries.}  $\tR = \IR$ is a first-class
  ...
  determines ledger credit.

\item \textbf{Interface invariance.}  CLI, Python API, REST API,
  C99 core, and C++ accelerator all produce identical results from
  the same spine and frozen parameters.
...
The simplicity of the example is a feature: the spine is identical
for three voltage readings and for 31~Standard Model
particles~\cite{paulus2025physicscoherence}.  Only the Tier-2
channel choice changes; the Tier-1 kernel and Tier-0 protocol are
invariant.
...
\texttt{git clone https://github.com/\\\hspace*{2em}calebpruett927/\\\hspace*{2em}GENERATIVE-COLLAPSE-DYNAMICS.git}\\[1pt]
...
We have provided a complete, self-contained trace of a single
measurement through the twelve stops of the GCD validation spine.
Every number is reproducible by running the open-source reference
implementation~\cite{umcpmetadatarepo}.  The paper serves as the
canonical entrance to the framework: a reader who has seen one
complete pass can navigate any domain closure, any theorem, and any
