...
\newcommand{\regime}[1]{\textrm{\scshape#1}}
...
effective degrees of freedom and how each output is computed; why
the frozen parameters are the unique values where validation
seams close and how they are discovered; why classical results
appear as degenerate limits and how the extra structure in GCD
surpasses them; why regimes are classified the way they are and
how the four-gate criterion partitions the manifold; and why the
seam calculus has monoid structure and how it debits drift,
credits return, and decides weld verdicts.  We begin from the
problem: the incoherence of standard scientific logic---the
arbitrary thresholds, unstated conventions, boolean verdicts, and
...
These are not complaints about any single field.  They are
structural features of the current scientific epistemology,
inherited from a period when measurement was local, manual, and
discipline-specific.  They persist because no alternative has been
proposed that resolves all five simultaneously.

\subsection{The resolution}

This paper presents the architecture of a system that resolves each
of these failures by construction.  The resolution is not a reform
of existing practice; it is a new structural foundation:
...
where you started; it means demonstrable re-entry into an
...
The corpus is organized into exactly three tiers.  No half-tiers,
no exceptions.  Every symbol, function, artifact, claim, and
definition belongs to exactly one tier.
...
\subsubsection{Drift (derived from $F$)}
\beq
...
\subsubsection{Integrity composite (derived from $\kappa$)}
\beq
...
Specifically, Jensen's inequality applied to the concave binary
entropy $h$ gives $S = \sum w_i h(c_i) \leq h(\sum w_i c_i) = h(F)$,
...
admissible return candidate was found.  Segments with $\tR = \IR$
receive zero return credit.
...
The orientation script (\S2--3) reproduces both results.
...
\noindent\textbf{Reproducible verification.}
The orientation script (\S8) reproduces this result: at
...
\noindent\textbf{What the classical result lacks.}
Unitarity~\cite{dirac1930principles,weinberg1995qft1} tells
you that probabilities sum to~1.  The duality identity tells
you \emph{where the probability went}: into fidelity (survival) or drift (loss), with
channel-level attribution via the weights, in a geometry
whose metric $g_F = 1/[c(1{-}c)]$ measures the cost of
distinguishing nearby states.  Remove the channel
attribution, remove the metric, remove the cost
interpretation---unitarity is what remains.

\noindent\textbf{Reproducible verification.}
The orientation script (\S1) confirms: across $10{,}000$
...
8 & Quantum mechanics & QDM, FQHE, entng. \\
...
\texttt{scripts/} directory: the deep diagnostic, both
cross-domain bridge phases, the identity verification suite,
and the identity deep probes.  The seam composition monoid
...
architecture loses a provable guarantee.  This is not a
philosophical assertion; it is tested by systematic component
removal (the load-bearing test~\cite{umcpmetadatarepo}).
...
\node[font=\scriptsize, text=black!50,
  anchor=north, text width=5.4cm, align=center]
  at (0, -1.0)
...
\begin{tikzpicture}
  ...
  \node[font=\scriptsize, align=center]
    at (0.4375, 0.6) {\textbf{S}\\[-2pt]
    {\tiny 12.5\%}};
  \node[font=\scriptsize, align=center]
    at (1.729, 0.6) {\textbf{Watch}\\[-2pt]
    {\tiny 24.4\%}};
  \node[font=\scriptsize, align=center]
    at (4.792, 0.6) {\textbf{Collapse}\\[-2pt]
    {\tiny 63.1\%}};

  % Threshold markers with tick lines below
  \draw[line width=0.5pt, black!50]
    (0.875, -0.08) -- (0.875, -0.35);
  \draw[line width=0.5pt, black!50]
    (2.583, -0.08) -- (2.583, -0.35);
  \node[font=\scriptsize, text=black!65, below]
    at (0.875, -0.35) {$\omega\!=\!0.038$};
  \node[font=\scriptsize, text=black!65, below]
    at (2.583, -0.35) {$\omega\!=\!0.30$};

  % Gate annotation — compact, below
  \node[font=\scriptsize, text=black!55,
