...
effective degrees of freedom (two exact algebraic constraints,
one asymptotic) and how each is computed; why the
...
44~structural identities, 47~lemmas, 5~frozen parameters,
5~structural constants, 4~regime gates, and 3~algebraic
identities that constitute the Tier-1 kernel and its Tier-0
protocol.  The corpus comprises 7{,}875~automated tests across
...
methods.
Section~\ref{sec:lemmas} inventories the 47~lemmas.
...
Two constraints ($\omega = 1 - F$, $\IC = e^\kappa$) are exact
algebraic identities; the third ($S \approx f(F, C)$) is an
asymptotic statistical relation that tightens with channel
count~$n$.
\end{theorem}
...
\begin{remark}[Return-rate calibration]
\label{rem:calibration}
The return rate $R$ is not a universal constant; it is a
\emph{closure-declared parameter} that each domain calibrates so
that the $\kappa$-space credit $R \cdot \tR$ is commensurate with
the actual integrity change $\Delta\kappa_{\mathrm{ledger}}$.
The \regime{FAIL} above is pedagogically intentional: it
demonstrates what happens when $R$ is left at the uncalibrated
unit default ($R = 1$).  To obtain a \regime{PASS}, the closure
must set $R$ so that $R \cdot \tR \approx
\Delta\kappa_{\mathrm{ledger}} + (D_\omega + D_C)$.

\medskip\noindent\textbf{Calibrated example.}
With $R = 0.132$ (calibrated to the $\kappa$-scale of this
trace): $\Delta\kappa_{\mathrm{budget}} = 0.132 \times 3
- (0.024 + 0.302) = 0.396 - 0.326 = 0.070$; residual
$s = 0.070 - 0.069 = 0.001$.  Since
$|s| = 0.001 < \tolseam = 0.005$, this weld \regime{PASS}es.
The uncalibrated example fails because $R = 1$ assigns three
full units of $\kappa$-credit for a transition that changes
$\kappa$ by only $0.069$---a 39-fold mismatch.  The seam
enforces proportionality: credit must match demonstrated change.
\end{remark}
...
% ============================================================
\section{The 47 Lemmas}\label{sec:lemmas}
% ============================================================
...
The 47~lemmas are not conjectures; each is verified
...
The full Rosetta table is given in Fig.~\ref{fig:rosetta}.
Integrity is never asserted within the Rosetta; it is
...
7{,}875~automated tests, and 406~scale-ladder objects spanning
...
61~orders of magnitude, and 7{,}875~automated tests.  It produces
...
% -------------------------------------------------------------
% Figure 7: The Rosetta — Cross-Domain Translation Table
% -------------------------------------------------------------
\begin{figure*}[t!]
\centering
\begin{tabular}{@{}llllll@{}}
\toprule
\textbf{Lens} & \textbf{Drift} & \textbf{Fidelity} & \textbf{Roughness} & \textbf{Return} & \textbf{Integrity} \\
\midrule
Epistemology &
  Change in belief/evidence &
  Retained warrant &
  Inference friction &
  Justified re-entry &
  \emph{(read from ledger)} \\[3pt]
Ontology &
  State transition &
...
Semiotics &
  ...
  Interpretant closure &
  \emph{(read from ledger)} \\
\bottomrule
\end{tabular}
\caption{The Rosetta: cross-domain translation of the five
words.  Each row is a \emph{lens}---an intellectual field
whose dialect is mapped to the fixed operational definitions
without altering them.  Integrity is never asserted within
the Rosetta; it is read from the reconciled ledger and
stance.  This is what makes cross-domain synthesis
mechanical: the prose maps through the Rosetta, while
$I = e^\kappa$ provides unitless multiplicative comparability
across seams.  See \S\ref{sec:canon} for the four-step
