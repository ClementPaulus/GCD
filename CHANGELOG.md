...
### Fixed
- Pinned all GitHub Actions to full-length commit SHAs (required by repo policy)
- Removed conda packaging (conda-verify incompatible with Python 3.12)
...
- PyPI publish: added `skip-existing: true` to prevent duplicate upload failures
- Version bump to 2.1.5 for fresh PyPI release with all latest content
